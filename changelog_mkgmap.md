...
## 01/02/2026
Removed `information=departure times and destinations` and `information=former_telephone_box`; no longer in the data.

## 11/01/2026
Detect crossings with flashing lights and append ('flas') if found.
Treat `rolled` and `sloped` as notable kerb values.
Add more `crossing` values to "is it or is it not a crossing" logic.
...
## 09/11/2025
Truncate crossing details appended to crossing and added some more.

## 06/11/2025
Detect general `hazard` and `overgrown` on highways.
Handle more synonyms for `level_crossing`.
Append crossing details to the crossing.
...
## 05/09/2025
Added an "ajt04" map style, as per "ajt03" but without most `footway=sidewalk`.
