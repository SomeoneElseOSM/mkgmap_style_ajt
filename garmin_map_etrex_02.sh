...
then
  ...
  java  -Xmx9600m -jar /usr/share/mkgmap-splitter/splitter.jar ../${file_prefix1}_${file_extension1}.osm.pbf --max-nodes=800000 --output=xml > /dev/null
fi
#
# ------------------------------------------------------------------------------
# Run mkgmap
...
# ------------------------------------------------------------------------------
echo "Running mkgmap"
java -Xmx9600m -jar /usr/share/mkgmap/mkgmap.jar --style-file=/home/${local_filesystem_user}/src/mkgmap_style_ajt/ajt  --add-pois-to-areas --remove-short-arcs --levels="0=24, 1=22, 2=21, 3=19, 4=18, 5=16" --location-autofill=3 --route --gmapsupp --overview-mapname=ajt02map --country-name="United Kingdom" --country-abbr="UK" --copyright-message="Copyright OpenStreetMap contributors" *.osm.gz > /dev/null
#
if [ -f gmapsupp.img ]; then
  ...
  mkdir -p /var/www/html/maps/mkgmap_maps/ajt02/${file_prefix1}/old
  #
  mv /var/www/html/maps/mkgmap_maps/ajt02/${file_prefix1}/ajt02_${file_prefix1}_supp.img /var/www/html/maps/mkgmap_maps/ajt02/${file_prefix1}/old/
  mv /var/www/html/maps/mkgmap_maps/ajt02/${file_prefix1}/ajt02_${file_prefix1}_map.tdb  /var/www/html/maps/mkgmap_maps/ajt02/${file_prefix1}/old/
  mv /var/www/html/maps/mkgmap_maps/ajt02/${file_prefix1}/ajt02_${file_prefix1}_map.img  /var/www/html/maps/mkgmap_maps/ajt02/${file_prefix1}/old/
  #
