...
#
# The output files produced are ajt04supp.img, ajt04map.tdb, ajt04map.img
...
# ------------------------------------------------------------------------------
if /home/${local_filesystem_user}/src/osm-tags-transform/build/src/osm-tags-transform -c /home/${local_filesystem_user}/src/mkgmap_style_ajt/transform_04.lua ../${file_prefix1}_${file_extension1}.osm.pbf -O -o transformed_after.pbf
then
    echo Transform OK
else
...
# --series-name="AJT04S" --family-name="AJT04F" --area-name="AJT04A" 
# --description="AJT04D" 
#
...
# ------------------------------------------------------------------------------
echo "Running mkgmap"
java -Xmx9600m -jar /usr/share/mkgmap/mkgmap.jar --style-file=/home/${local_filesystem_user}/src/mkgmap_style_ajt/ajt03  --add-pois-to-areas --remove-short-arcs --levels="0=24, 1=22, 2=21, 3=19, 4=18, 5=16" --location-autofill=3 --route --gmapsupp --overview-mapname=ajt04map --country-name="United Kingdom" --country-abbr="UK" --copyright-message="Copyright OpenStreetMap contributors" *.osm.gz > /dev/null
#
if [ -f gmapsupp.img ]; then
  mkdir -p etrex
  mv 6???????.img etrex
  mv gmapsupp.img  etrex/ajt04_${file_prefix1}_supp.img
  mv ajt04map.tdb  etrex/ajt04_${file_prefix1}_map.tdb
  mv ajt04map.img  etrex/ajt04_${file_prefix1}_map.img
  # ---------------------------------------------------------------------------
  # At this point we know that a file was successfully created.
  # Copy it to a directory below a webserver /var/www/html/maps/mkgmap_maps/ajt04:
  # ---------------------------------------------------------------------------
  mkdir -p /var/www/html/maps/mkgmap_maps/ajt04/${file_prefix1}
  mkdir -p /var/www/html/maps/mkgmap_maps/ajt04/${file_prefix1}/old
  #
  mv /var/www/html/maps/mkgmap_maps/ajt04/${file_prefix1}/ajt04_${file_prefix1}_supp.img /var/www/html/maps/mkgmap_maps/ajt04/${file_prefix1}/old/
  mv /var/www/html/maps/mkgmap_maps/ajt04/${file_prefix1}/ajt04_${file_prefix1}_map.tdb  /var/www/html/maps/mkgmap_maps/ajt04/${file_prefix1}/old/
  mv /var/www/html/maps/mkgmap_maps/ajt04/${file_prefix1}/ajt04_${file_prefix1}_map.img  /var/www/html/maps/mkgmap_maps/ajt04/${file_prefix1}/old/
  #
  cp etrex/ajt04_${file_prefix1}_supp.img /var/www/html/maps/mkgmap_maps/ajt04/${file_prefix1}/
  cp etrex/ajt04_${file_prefix1}_map.tdb /var/www/html/maps/mkgmap_maps/ajt04/${file_prefix1}/
  cp etrex/ajt04_${file_prefix1}_map.img /var/www/html/maps/mkgmap_maps/ajt04/${file_prefix1}/
  # ---------------------------------------------------------------------------
